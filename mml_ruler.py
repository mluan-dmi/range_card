...
- `/ruler [label="1 mile"] [pct=0.25] [border=96] [align=left|center|right]` — add a bottom border and draw a scale bar.
...
def add_border_and_ruler(
    ...
    """
    Append a bottom border to the image and draw a scale bar of length `pct` * width,
    with end ticks, subdivided ticks, and a text label.
    """
    img = Image.open(path).convert("RGB")
    w, h = img.size

    pct = max(0.05, min(0.9, float(pct)))
    border_px = max(24, int(border_px))
    ...
    # Border divider line
    draw.line([(0, h), (w, h)], fill=(220, 220, 220), width=1)

    # Ruler geometry
    margin = int(max(12, w * 0.02))
    bar_len = int(round(w * pct))
    bar_len = max(60, min(w - 2 * margin, bar_len))
    bar_y = h + border_px // 2
    stroke = max(4, border_px // 12)
    tick_h = max(8, stroke * 2)
    sub_tick_h = max(6, stroke)
    ...
    # Main bar
    draw.rectangle([x0, bar_y - stroke // 2, x1, bar_y + stroke // 2], fill=(0, 0, 0))

    # End ticks
    draw.line([(x0, bar_y - tick_h), (x0, bar_y + tick_h)], fill=(0, 0, 0), width=1)
    draw.line([(x1, bar_y - tick_h), (x1, bar_y + tick_h)], fill=(0, 0, 0), width=1)

    # Subdivisions (quartiles)
    for f in (0.25, 0.5, 0.75):
        xt = int(round(x0 + f * bar_len))
        draw.line([(xt, bar_y - sub_tick_h), (xt, bar_y + sub_tick_h)], fill=(0, 0, 0), width=1)

    # Label
    try:
        font = ImageFont.load_default()
        tw, th = draw.textbbox((0, 0), label, font=font)[2:]
        tx = x0 if align == "left" else (x0 + (bar_len - tw) // 2 if align == "center" else x1 - tw)
        ty = bar_y - tick_h - th - 6
        draw.text((tx, ty), label, fill=(0, 0, 0), font=font)
    except Exception:
        pass

    out_name = f"{stem}_ruler_{label.replace(' ', '_')}_{pct:.2f}_{border_px}_{align}{suffix}"
    out_path = outdir / out_name
    canvas.save(out_path)
    return out_path


def parse_kv_tokens(tokens: List[str]) -> dict:
    """
    Parse key=value tokens where value may be quoted: key="some value"
...
def run_loop(
    ...
    print("Model loaded.")
    print("Commands: /mode builder | /mode prompt | /sanity on|off | /sanity_box PCT | /mark x y | /undo | /reset | /radius N | /image PATH | /describe_* ... | /ruler [label=\"1 mile\"] [pct=0.25] [border=96] [align=left|center|right] | exit")
    ...
    while True:
        ...
        # ---- Ruler command ----
        if lower.startswith("/ruler"):
            # Parse optional key=value tokens
            parts = user_input.split()[1:]
            kv = parse_kv_tokens(parts)
            label = kv.get("label", "1 mile")
            pct = float(kv.get("pct", 0.25))
            border = int(float(kv.get("border", 96)))
            align = kv.get("align", "left")
            try:
                out = add_border_and_ruler(current, label=label, pct=pct, border_px=border, align=align, outdir=outdir)
                history.append(out)
                current = out
                _append_log(log_path, {"ts": _now(), "mode": "ruler", "label": label, "pct": pct, "border_px": border, "align": align, "output": str(out)})
                print(f'Added border + ruler (“{label}”, {pct*100:.1f}% width, border={border}px, {align}). Saved → {out}')
            except Exception as e:
        ...
        if lower == "/undo":
            if history:
                history.pop()
                # Only decrement step if the last step actually added a point
                if step in points:
                    points.pop(step, None)
                    step -= 1
                current = history[-1] if history else original
                print(f"Undid last change. Current image → {current}")
            else:
        ...
        if sanity_check and used_target:
            try:
                ...
                sanity_prompt = f'Is {used_target} in this image? Answer "YES" or "NO" and briefly justify.'
